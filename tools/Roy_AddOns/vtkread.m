function surface = read_vtk(filename, read_scalars, verbose)
...
if(info == 'V')
    ...
    A = reshape(A, 2, cnt/2);
    surface.faces = repmat(A(2,:)+1, 3, 1);
end
...
if(read_scalars ~= 0) 
    ...
    info = sscanf(str,'%s %*s %*s', 1);
    if strcmp(info,'POINT_DATA')
        str = fgets(fid);
        info = sscanf(str,'%s %*s %*s', 1);
    end
    surface.scalarname = sscanf(str,'%*s %s %*s', 1);
    str = fgets(fid);
